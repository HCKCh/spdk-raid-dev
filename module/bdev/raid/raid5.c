...
#define __CHUNK_IN_RANGE(req, c) \
	c < req->chunks + raid5_ch_to_r5f_info(req->r5ch)->raid_bdev->num_base_bdevs
...
raid5_ch_to_r5f_info(struct raid5_io_channel *r5ch)
{
...
static void
raid5_xor_small_write_stripe_continue(struct stripe_request *stripe_req, uint8_t n_src)
{
	struct raid5_io_channel *r5ch = stripe_req->r5ch;
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
...
raid5_xor_small_write_stripe(struct stripe_request *stripe_req, stripe_req_xor_cb cb)
{
	...
	raid5_xor_small_write_stripe_continue(stripe_req, c);
}
...
static int
raid5_small_write_stripe_request_map_iovecs(struct stripe_request *stripe_req)
{
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	struct raid5_info *r5f_info = raid_bdev->module_private;
	struct chunk *chunk;
...
raid5_submit_small_write_request(struct raid_bdev_io *raid_io, uint64_t stripe_index)
{
	...
	raid5_stripe_small_write_request_init(stripe_req, raid_io, stripe_index, raid_io->num_blocks);

	ret = raid5_small_write_stripe_request_map_iovecs(stripe_req);
	if (spdk_unlikely(ret)) {
		return ret;
	}
	...
	if (raid_bdev_channel_get_base_channel(raid_io->raid_ch, stripe_req->parity_chunk->index) != NULL) {
		...
		raid5_xor_small_write_stripe(stripe_req, raid5_stripe_write_request_xor_done);
	} else {
...
static int
raid5_submit_full_stripe_write_request(struct raid_bdev_io *raid_io, uint64_t stripe_index)
{
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	struct raid5_io_channel *r5ch = raid_bdev_channel_get_module_ctx(raid_io->raid_ch);
	struct stripe_request *stripe_req;
	int ret;
	printf("[raid5_submit_full_stripe_write_request] START!!!\n");
	stripe_req = TAILQ_FIRST(&r5ch->free_stripe_requests.write);
	if (!stripe_req) {
		return -ENOMEM;
	}
...
raid5_submit_rw_request(struct raid_bdev_io *raid_io)
{
	...
	case SPDK_BDEV_IO_TYPE_WRITE:
		...
		if(stripe_offset == 0 && raid_io->num_blocks == r5f_info->stripe_blocks) {
			...
			ret = raid5_submit_small_write_request(raid_io, stripe_index);
		}
		//ret = raid5_submit_full_stripe_write_request(raid_io, stripe_index);
		break;
	default:
...
raid5_stripe_request_free(struct stripe_request *stripe_req)
{
	...
	if (stripe_req->type == STRIPE_REQ_WRITE) {
		...
		struct raid5_info *r5f_info = raid5_ch_to_r5f_info(stripe_req->r5ch);
...
{
	struct raid5_info *r5f_info = raid5_ch_to_r5f_info(r5ch);
	struct raid_bdev *raid_bdev = r5f_info->raid_bdev;
...
{
	struct raid5_io_channel *r5ch = ctx_buf;
	struct raid5_info *r5f_info = io_device;
	struct raid_bdev *raid_bdev = r5f_info->raid_bdev;
	struct stripe_request *stripe_req;
	int i;
...
err:
	...
	raid5_ioch_destroy(r5f_info, r5ch);
	return -ENOMEM;
}
...
raid5_start(struct raid_bdev *raid_bdev)
{
	...
	r5f_info->stripe_blocks = raid_bdev->strip_size * raid5_stripe_data_chunks_num(raid_bdev);
	...
	spdk_io_device_register(r5f_info, raid5_ioch_create, raid5_ioch_destroy,
...
raid5_stop(struct raid_bdev *raid_bdev)
{
	...
	spdk_io_device_unregister(r5f_info, raid5_io_device_unregister_done);
